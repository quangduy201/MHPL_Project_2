...
public class CurveLineChart extends JComponent {
    ...
    private void createBlankChart() {
        blankPlotChart = new BlankPlotCurveLineChart();
        add(blankPlotChart);
    }

    private void createChart() {
        blankPlotChart.setBlankPlotChatRender(new BlankPlotCurveLineChatRender() {
            ...
            @Override
            public void renderGraphics(BlankPlotCurveLineChart chart, Graphics2D g2, Rectangle2D rectangle) {
    ...
    public void addLegend(String name, Color color1, Color color2) {
        ModelCurveLineLegend data = new ModelCurveLineLegend(name, color1, color2);
        legends.add(data);
    ...
    private void clearLegendSelected(CurveLineLegendItem item) {
        ...
        for (Component com : panelLegend.getComponents()) {
            if (com instanceof CurveLineLegendItem) {
                CurveLineLegendItem l = (CurveLineLegendItem) com;
                if (l != item) {
                    l.setSelected(false);
                }
            }
    ...
    public void addData(ModelCurveLineChart data) {
        model.add(data);
    ...
    private BlankPlotCurveLineChart blankPlotChart;
